...
# MODEL: SUPPORT VECTOR MACHINE (SVM) - KERNEL = RBF 
# In[30]:
...
# ============================
# Apply SVM with an RBF kernel to classify tumor types as benign or malignant
from sklearn.svm import SVC
svm_rbf_model = SVC(kernel='rbf')
evaluate_model(svm_rbf_model, "SVM (RBF Kernel)")

# ============================
# 6. MODEL PERFORMANCE SUMMARY FOR SVM (RBF KERNEL)
# ============================
...
svm_rbf_results = {
    ...
    'Accuracy': [accuracy_score(y_test, svm_rbf_model.predict(X_test))],
    'Confusion Matrix': [confusion_matrix(y_test, svm_rbf_model.predict(X_test))]
}

results_df = pd.DataFrame(svm_rbf_results)
print("Summary of SVM (RBF Kernel) Model Performance:")
print(results_df)

# Optional: Save results to CSV
# results_df.to_csv("svm_rbf_results_summary.csv", index=False)
